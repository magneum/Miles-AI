...
# ========================================================= Magneum =========================================================
def Hyper_Builder(hp, input_shape, num_classes, patience):
    model = Sequential()
    model.add(
        Conv2D(
    ...
    model.add(
        Dropout(rate=hp.Float("dropout", min_value=0.1, max_value=0.5, default=0.25))
    )
    for i in range(hp.Int("num_blocks", 1, 4, default=2)):
        ...
        model.add(
            Dropout(
                rate=model.layers[-2].output.get_shape().as_list()[1]
