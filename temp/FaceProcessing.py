...
print(f"{Fore.YELLOW}{Style.BRIGHT}Code Description:")
...
print(
    f"\n{Fore.CYAN}{Style.BRIGHT}The RandomSearch tuner is then created with the build_model() function as the model-building function, and the maximum number of trials, project name, and objective for tuning (in this case, 'val_accuracy') are specified. The tuner searches for the best hyperparameters using the search() function, which takes the input data (X_Index and Y_Index), number of epochs, batch size, and other parameters."
)
print(
...
if not os.path.exists(_path):
    ...
    print(
        Fore.YELLOW + Style.BRIGHT + f"Folder already exists: {_path}" + Style.RESET_ALL
    )
...
print(Fore.YELLOW + Style.BRIGHT + f"Data shape: {Fer2013.shape}")
print(Fore.YELLOW + Style.BRIGHT + f"Columns: {', '.join(Fer2013.columns)}")
print(Style.RESET_ALL)
...
print(Fore.YELLOW + Style.BRIGHT + f"Total number of samples: {X_Index.shape[0]}")
print(Fore.YELLOW + Style.BRIGHT + f"Total number of features: {X_Index.shape[1]}")
print(Fore.YELLOW + Style.BRIGHT + f"Total number of labels: {Y_Index.shape[0]}")
print(Fore.CYAN + Style.BRIGHT + f"Number of epochs: {nEpochs}")
print(Fore.CYAN + Style.BRIGHT + f"Validation split: {nValsplit}")
print(Style.RESET_ALL)
...
def build_model(hp):
    ...
    print(Fore.CYAN + Style.BRIGHT + f"filters_1: {hp.get('filters_1')}")
    print(Fore.CYAN + Style.BRIGHT + f"kernel_size_1: {hp.get('kernel_size_1')}")
    print(Fore.CYAN + Style.BRIGHT + f"pool_size_1: {hp.get('pool_size_1')}")
    for i in range(1, hp.get("nblocks") + 1):
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f"filters_{i+1}: {hp.get('filters_' + str(i + 1))}"
        )
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f"kernel_size_{i+1}: {hp.get('kernel_size_' + str(i + 1))}"
        )
        print(
            Fore.CYAN
            + Style.BRIGHT
            + f"pool_size_{i+1}: {hp.get('pool_size_' + str(i + 1))}"
        )
    print(Fore.CYAN + Style.BRIGHT + f"units: {hp.get('units')}")
    print(Fore.CYAN + Style.BRIGHT + f"learning_rate: {hp.get('learning_rate')}")
    print(Style.RESET_ALL)
...
print(Fore.YELLOW + Style.BRIGHT + f"filters_1: {BestHP.get('filters_1')}")
print(Fore.YELLOW + Style.BRIGHT + f"kernel_size_1: {BestHP.get('kernel_size_1')}")
print(Fore.YELLOW + Style.BRIGHT + f"pool_size_1: {BestHP.get('pool_size_1')}")
print(Fore.YELLOW + Style.BRIGHT + f"nblocks: {BestHP.get('nblocks')}")
for i in range(BestHP.get("nblocks")):
    print(
        Fore.YELLOW + Style.BRIGHT + f"filters_{i+2}: {BestHP.get('filters_'+str(i+2))}"
    )
    print(
        Fore.YELLOW
        + Style.BRIGHT
        + f"kernel_size_{i+2}: {BestHP.get('kernel_size_'+str(i+2))}"
    )
    print(
        Fore.YELLOW
        + Style.BRIGHT
        + f"pool_size_{i+2}: {BestHP.get('pool_size_'+str(i+2))}"
    )
print(Fore.YELLOW + Style.BRIGHT + f"units: {BestHP.get('units')}")
print(Fore.YELLOW + Style.BRIGHT + f"learning_rate: {BestHP.get('learning_rate')}")
print(Style.RESET_ALL)
