...
def build_model(hp):
    ...
    model.add(
        layers.Dense(
            ...
            if hp.Boolean("use_L1_regularization")
            else None,
            bias_regularizer=l2(
                hp.Choice("l2_regularization", values=[0.0, 1e-5, 1e-4])
            )
            if hp.Boolean("use_L2_regularization")
            else None,
    ...
    learning_rate = ExponentialDecay(
        ...
        decay_steps=10000,
        decay_rate=hp.Float("decay_rate", min_value=0.1, max_value=1, sampling="log"),
    )
    optimizer = keras.optimizers.Adam(learning_rate)
    model.compile(
        optimizer=optimizer, loss="binary_crossentropy", metrics=["accuracy", "mae"]
    )
...
model.search(
    ...
    callbacks=[tensorflow.keras.callbacks.EarlyStopping(patience=1)],
)
